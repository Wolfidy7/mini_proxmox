...
class virDomain(object):
    ...
    def abortJobFlags(self, flags=0):
        """Requests that the current background job be aborted at the
        soonest opportunity. In case the job is a migration in a post-copy mode,
        ...
        ret = libvirtmod.virDomainAbortJobFlags(self._o, flags)
        if ret == -1:
            raise libvirtError('virDomainAbortJobFlags() failed')
        return ret
    ...
    def saveParams(self, params, flags=0):
        """This method will suspend a domain and save its memory contents to a file on disk. """
        ret = libvirtmod.virDomainSaveParams(self._o, params, flags)
        if ret == -1:
...
class virConnect(object):
    ...
    def restoreParams(self, params, flags=0):
        """This method will restore a domain saved to disk. """
        ret = libvirtmod.virDomainRestoreParams(self._o, params, flags)
        if ret == -1:
            raise libvirtError('virDomainRestoreParams() failed')
        return ret
...
VIR_DOMAIN_IOTHREAD_THREAD_POOL_MAX = "thread_pool_max"
VIR_DOMAIN_IOTHREAD_THREAD_POOL_MIN = "thread_pool_min"
...
VIR_DOMAIN_SAVE_PARAM_DXML = "dxml"
VIR_DOMAIN_SAVE_PARAM_FILE = "file"
